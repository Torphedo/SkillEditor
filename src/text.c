...
skill_text get_skill_text(pd_meta p, unsigned int id) {
    ...
    const gsdata* gstorage = p.gstorage.local_data;
...
void shift_textbuf(pd_meta p, char* offender, s32 new_len, u32 offender_id, bool is_name) {
    ...
    const u64 remaining_space = sizeof(gstorage->textbuf) - (u64)(offender - gstorage->textbuf);
    ...
    for (u32 i = offender_id + 1; i < ARRAYSIZE(gstorage->textPtrs); i++) {
